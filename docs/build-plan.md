...
- ElectriPy AI 0.5.0 is currently distributed as Python 3.11+, not as a TypeScript package. The TypeScript application therefore preserves a generic `AiRuntimePort` and implements bounded timeout/retry/circuit behavior plus safe telemetry locally. This is not an active ElectriPy integration; a Python sidecar is intentionally excluded from the one-day critical path.
