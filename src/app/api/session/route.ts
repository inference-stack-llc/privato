...
import { DEMO_IDENTITY_COOKIE } from "@/modules/identity/session";
...
export async function POST(request: Request) {
  ...
  response.cookies.set(DEMO_IDENTITY_COOKIE, parsed.data.memberId, { httpOnly: true, sameSite: "strict", secure: process.env.NODE_ENV === "production", path: "/", maxAge: 60 * 60 * 8 });
  return response;
}
